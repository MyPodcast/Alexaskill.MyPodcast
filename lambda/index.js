...
const LaunchRequestHandler = {
    ...
    async handle(handlerInput) {
        ...
        var podcasts = await getPodcasts(gist.raw_url);
        let names = "";
        let state = "";
        let number = 0;
            ...
            if (podcasts[i+1]!==null){
                ...
                return handlerInput.responseBuilder
                  .addAudioPlayerPlayDirective
                           ('ENQUEUE', nextpodcast.url, nextpodcast.token, nextpodcast.offset, podcasts[i-1].token)
                  .getResponse();
            } else {
                return handlerInput.responseBuilder
...
// ----------------------------------------------------------------------------------------------------------------
// UpdatePodcastsHandler : Mise à jour du podcast via developper console à partir du fichier podscasts.json ASUPPRIMER QUAND  WEB OK
// ----------------------------------------------------------------------------------------------------------------
const UpdatePodcastsHandler = {
    canHandle(handlerInput) {
    ...
    async handle(handlerInput) {
        ...
        var gist = await getGist(token);
        var podcasts = await getPodcasts(gist.raw_url);
        podcasts  =  podcasts_up;
        UpdatePodcasts(podcasts, gist.url, token);
        return handlerInput.responseBuilder
            .speak(`Mise à jour des podcasts terminée vous pouvez me demander maintenant la liste`)
            .withSimpleCard('Podcasts', `Mise à jour`)
            .getResponse();
    },
};
...
exports.handler = Alexa.SkillBuilders.custom()
    .addRequestHandlers(
        ...
        CheckAccountPermissionHandler, //désactiver si réactivation penser a remettre les permission ...
